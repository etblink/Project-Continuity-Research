# CCP-1 Frozen-Snapshot Current Orientation Projection — Blind Audit Packet 0.1.3
...
The candidate must describe project orientation at exact frozen source snapshot:
...
Determine whether candidate `CURRENT_ORIENTATION.md` faithfully summarizes the frozen snapshot while remaining explicitly non-canonical.

Check:
1. project purpose / North Star;
2. immediate objective of the current phase;
3. current phase;
4. completed vs remaining gates;
5. architecture-candidate status;
6. authorization/prohibition boundaries;
7. bounded next operation at that snapshot;
8. machine checkpoint and limitations;
9. evidence/navigation mapping;
10. historical/current distinction;
11. projection-vs-source-of-truth boundary.

For the immediate-objective check, reject if the candidate turns a conditional advancement question into a foregone decision, claims CCP-2 authorization, or overstates what cold-start success would prove.
