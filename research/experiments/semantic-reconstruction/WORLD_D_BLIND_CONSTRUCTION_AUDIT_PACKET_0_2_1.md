# World D Blind Construction Audit Packet 0.2.1
...
- any prior construction audit of this world;
...
Do not solve against a hidden answer key. Test whether the corpus itself supports a uniquely recoverable operative project model and requires genuine integration across records.

### A. Reconstruct the operative model
...
9. unresolved uncertainties and their exact scope;
10. the substantive next-action family that satisfies all explicit constraints.

### B. Ambiguity attack

Try to construct two materially different next-action families that both satisfy every explicit governing constraint.

If you can, mark `UNDER_SPECIFIED`.

Do not count ordinary implementation variants within one substantive continuation family as under-specification.

### C. Hidden-preference attack

Ask whether distinguishing faithful from unfaithful continuation requires an unstated preference such as speed vs quality, conservatism vs progress, stakeholder importance, risk tolerance, aesthetics, or assumed organizational practice.

If yes, mark `HIDDEN_PREFERENCE_DEPENDENCY`.

### D. Missing-information attack

Identify any decision-relevant fact that is absent but necessary to distinguish faithful from unfaithful continuation.
...
Explain which non-adjacent records must actually be integrated to recover:
- project purpose;
- workstream boundaries;
- authority;
- exception state;
- unresolved obligations;
- the next-action family.
...
Distinguish harmless latitude within one continuation family from latitude that would make the core next action non-unique.
